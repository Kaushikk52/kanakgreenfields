...
export default function Card({ 
  ...
  return (
    <div className="w-[250px] md:min-w-[500px] md:min-h-[650px]  h-[350px]  rounded-lg overflow-hidden relative">
      ...
      {mode === 'top' && (
        <div className="absolute inset-0 p-10 bg-gradient-to-b from-35% from-[#457f5e] to-transparent flex flex-col justify-start  text-white">
      ...
      {mode === 'bottom' && (
        <div className="absolute inset-0 p-10 bg-gradient-to-t from-35% from-[#457f5e] to-transparent flex flex-col justify-end  text-white">
          <h1 className="text-4xl font-bold">{title}</h1>
          <p className="md:text-2xl text-lg mt-10">{description}</p>
        </div>
      )}
